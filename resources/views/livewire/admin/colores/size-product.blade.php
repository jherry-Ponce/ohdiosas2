    ...
    <ul class="mt-12 space-y-4">
            ...
            @foreach ($sizes as $size)
                <li class="bg-white shadow-lg rounded-lg p-6" wire:key="size-{{ $size->id }}">
                    ...
                    @if ($size->id)
                         @livewire('admin.colores.color-size', ['size' => $size], key('color-size-'.$size->id))
                    @endif
                </li>
            @endforeach
...
@push('script')
<script>
    Livewire.on('deleteSize', pivot =>{
    Swal.fire({
        title: 'Estas seguro?',
        text: "No podrás revertir estosize product!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            Livewire.emit('deleteSizes',pivot);
            Swal.fire(
            'Eliminado!',
